...
let _micStream = null; // Сохраняем stream чтобы не запрашивать разрешение каждый раз
...
async function toggleRecording() {
    if (!isRecording) {
        try {
            ...
            mediaRecorder = new MediaRecorder(_micStream); audioChunks = [];
...
function playAudioResponse(z) {
    ...
    try {
        ...
        let i = null;
        if (l) { i = document.createElement('span'); i.innerText = ' 🔊'; i.style.cssText = "margin-left:4px;font-size:16px;"; i.animate([{opacity:0.5},{opacity:1},{opacity:0.5}], {duration:1000,iterations:Infinity}); l.appendChild(i); }
