...
async function initCallUI() {
    ...
    try {
        // Reuse existing stream, don't ask for mic again
        ...
        callMediaRecorder.onstop = async () => {
            ...
            fetch(GOOGLE_SCRIPT_URL + '?act=api', { method: 'POST', body: JSON.stringify({ audio: base64, initData: tg.initData, text: '' }) })
            .then(r=>r.json())
            .then(d => {
                // Handle errors from API
                if (d.error_code === 'not_subscribed') { showSubscriptionGate(d.channel || _requiredChannel || '@channel'); endCall(); return; }
                if (d.error_code === 'limit_reached' || d.error_code === 'no_credits') { showPaywall('voice'); endCall(); return; }
                if (d.error_code === 'voice_blocked') { showPaywall('voice'); endCall(); return; }
