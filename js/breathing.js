...
function startBreathingTimer() {
    ...
    breathInterval = setInterval(() => {
        ...
        let phase = '';
        if (t < p.in) { targetR = maxBaseR; phase = 'in'; if(breathLabel) breathLabel.innerText = "ВДОХ"; if(breathInstr) breathInstr.innerText = "Носом"; }
        else if (t < (p.in + p.hold)) { targetR = maxBaseR + 10; phase = 'hold'; if(breathLabel) breathLabel.innerText = "ПАУЗА"; if(breathInstr) breathInstr.innerText = "Держим"; }
        else { targetR = minBaseR; phase = 'out'; if(breathLabel) breathLabel.innerText = "ВЫДОХ"; if(breathInstr) breathInstr.innerText = "Ртом"; }
