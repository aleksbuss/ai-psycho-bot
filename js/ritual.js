...
function initRitual() {
    ...
    if (icon) icon.textContent = ritualSteps === MORNING_STEPS ? '🌅' : '🌙';
    if (title) title.textContent = ritualSteps === MORNING_STEPS ? 'Утренний ритуал' : 'Вечерний ритуал';
    if (desc) desc.textContent = 'Несколько минут для себя. Готовы?';
